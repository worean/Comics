...
from img2imgDataset import server_ImageAndRefDataset
...
class Painter:
    ...
    def colorize(self, id_str, step='C', blur=0, s_size=128,colorize_format="jpg"):
        ...
        image_out_path = {
            ...
            'C': self.outdir + id_str + "." + colorize_format}
        self.save_as_img(image_conv2d_layer.data[0], image_out_path[step])
        del image_conv2d_layer
    def colorize_local(self, sketch, mask, id_str, isSave=True, step='C', blur=0, s_size=128,colorize_format="jpg"):
        if self.gpu >= 0:
            cuda.get_device(self.gpu).use()
        dataset = server_ImageAndRefDataset(
            [id_str + ".png"], sketch, mask)
        ...
        image_out_path = {
            'S': self.outdir_min + id_str + ".png", 
            'L': self.outdir + id_str + ".jpg", 
            'C': self.outdir + id_str + "_colorized." + colorize_format}
        if isSave:
            self.save_as_img(image_conv2d_layer.data[0], image_out_path[step])
        else:
            self.cvtImage(image_conv2d_layer.data[0])

        del image_conv2d_layer

    def setOutdir(self, outPath = "out/", outminPath = "out_min/"):
        self.outdir = self.root + outPath
        self.outdir_min = self.root+ outminPath

    def cvtImage(self, img_conv2d_data):
        array = img_conv2d_data.transpose(1, 2, 0)
        array = img_conv2d_data.clip(0, 255).astype(np.uint8)
        array = cuda.to_cpu(array)
        (major, minor, _) = cv2.__version__.split(".")
        if major == '3':
            img = cv2.cvtColor(array, cv2.COLOR_YUV2RGB)
