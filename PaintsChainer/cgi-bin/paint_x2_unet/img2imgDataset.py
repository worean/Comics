...
class server_ImageAndRefDataset(chainer.dataset.DatasetMixin):

    def __init__(self, paths, img_line, img_mask, dtype=np.float32):
        self._paths = paths
        self._img_line = img_line
    ...
    def get_example(self, i, minimize=False, blur=0, s_size=128256):
        #image1 = ImageDataset._read_image_as_array(path1, self._dtype)

        image1 = cv2.cvtColor(self._img_line, cv2.COLOR_BGR2GRAY)
        print("load line image" )
...
class ImageAndRefDataset(chainer.dataset.DatasetMixin):
    ...
    def get_example(self, i, minimize=False, blur=0, s_size=256):
